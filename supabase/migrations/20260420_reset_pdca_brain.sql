-- Reset controlado da base do projeto PDCA Brain
-- Mantem schemas internos do Supabase (auth, storage, extensions) e
-- reinicia apenas os objetos da aplicacao em public.

drop trigger if exists trg_set_updated_at_pdca_brain on public.pdca_brain;
drop function if exists public.set_updated_at_pdca_brain();
drop table if exists public.pdca_brain cascade;
